...
const NewSales = ({ invoice }) => {
  ...
  const fetchStockData = async (productId) => {
    try {
      ...
      if (response.ok) {
        ...
        setFormData(prevData => ({
          ...prevData,
          productPrice: stockData.unitPrice
        }));
      } else {
  ...
  const handleCellEdit = (rowIndex, cellIndex, newValue) => {
    ...
    const qty = parseFloat(updatedTableData[rowIndex][3]) || 1;
    const discount = parseFloat(updatedTableData[rowIndex][4]) || 0;

    // Calculate new total price
    const totalPrice = (productPrice - discount) * qty;
    updatedTableData[rowIndex][5] = totalPrice.toFixed(2);
